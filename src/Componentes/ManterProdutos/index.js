...
export function ManterProdutos({ navigation, route }) {
    ...
    const [nomeCobertura, setCobertura] = useState(route?.params?.nomeValidade)
    ...
    async function salvarProduto() {
        if (nomeProduto == '') {
            ...
            let resultado = ''
            if (route?.params) {
                resultado = await atualizarProdutos(route?.params?.id, { nomeProduto, precoProduto, nomeCobertura })
            }
            else {
                resultado = await cadastrarProdutos({ nomeProduto, precoProduto, nomeCobertura })
            }
            if (resultado == 'erro') {
    ...
    return (
        <View style={estilos.container}>
            ...
            <TextInput
                ...
                mode="outlined"
                error={statusErro == 'Cobertura'}
                style={estilos.input} />
            {statusErro == 'Cobertura' ? <HelperText type="error" visible={statusErro == 'Cobertura'}>
                {mensagemErro}
                </HelperText> : null}
            <TouchableOpacity
